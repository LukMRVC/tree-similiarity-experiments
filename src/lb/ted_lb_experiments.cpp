...
ResultPair execute_sed_bounded_lb(TreeCollection &, Candidates &, int threshold);
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        ...
        std::cerr << "Usage: \n";
        std::cerr
                << "\t ted-lb-experiments DATASET_PATH RESULTS_PATH [bib|sed|sed_k|lblint|lblint_ix|hist|tang] THRESHOLD [?OUTPUT_PATH]\n\n";
        exit(1);
    }
    ...
    if (lb_alg == "bib") {
        ...
        times = execute_sed_bounded_lb(trees_collection, candidates, threshold);
        ...
        execute_sed_lb(trees_collection, candidates, threshold);
        times = execute_sed_bounded_lb(trees_collection, candidates2, threshold);
        if (candidates2.size() != candidates.size()) {
            std::cerr << "Sizes of candidate results are not the same!";
            exit(0);
        }
        for (auto i = 0; i < candidates.size(); ++i) {
            if (candidates[i] != candidates2[i]) {
                std::cerr << "Difference in candidates: \n";
                std::cerr << "[" << candidates[i].first << ", " << candidates[i].second << "] != ";
                std::cerr << "[" << candidates2[i].first << ", " << candidates2[i].second << "]\n";
                exit(0);
            }
        }
        std::cout << "Candidates are the same!\n";
    } else {
    ...
    auto correct = 0;
    auto incorrect = 0;
    std::ofstream candidates_output("/dev/null");
    if (!output_path.empty()) {
        candidates_output.close();
        candidates_output.open(output_path);
    }
    for (const auto &pair: candidates) {
        ...
        candidates_output << pair.first << "," << pair.second << "\n";
    }
    candidates_output.close();
    std::cout << "Correct trees;Incorrect trees;Precision;Total Time\n";
    std::cout << correct << ";" << incorrect << ";" << 100 * ((double) correct / (double) (correct + incorrect)) << ";"
              << times.first.count() << "\n";
...
ResultPair execute_sed_bounded_lb(TreeCollection &collection, Candidates &candidates, int threshold) {
    LabelDictionary ld;
    CostModelLD ucm(ld);
    std::cout << "Computing tree indexes\n";
    // preprocess - index tree collection
    ...
    for (int i = 0; i < tree_indexes.size(); i++) {
        ...
        auto ted_start = high_resolution_clock::now();
        for (int j = i + 1; j < tree_indexes.size(); j++) {
            auto lb = sed_ti.ted_k(tree_indexes[i], tree_indexes[j], threshold + 1);
            if (lb <= threshold) {
                candidates.emplace_back(std::make_pair(i, j));
            }
        }
    ...
    std::cout << "Only  Bounded-SED time: " << duration_cast<std::chrono::milliseconds>(total_ted_time).count()
              << "ms\n";
    return std::make_pair(total_exec_time, ted_times);
}

ResultPair
execute_label_intersection_lb(TreeCollection &collection, Candidates &candidates, int threshold) {
    LabelDictionary ld;
    CostModelLD ucm(ld);
    // preprocess - index tree collection
...
//        std::cout << "Processing tree " << i << " of " << tree_indexes.size() << "\n";
    ...
    total_exec_time = duration_cast<std::chrono::milliseconds>(
            high_resolution_clock::now() - total_exec_time_start);
...
ResultPair execute_label_intersection_index_lb(TreeCollection &collection, Candidates &candidates, int threshold) {
    ...
    auto total_exec_time = duration_cast<std::chrono::milliseconds>(
            high_resolution_clock::now() - total_exec_time_start);
...
ResultPair execute_hist_lb(TreeCollection &collection, Candidates &candidates, int threshold) {
    ...
    auto preprocess_start = high_resolution_clock::now();
    hc.create_histogram(collection, label_histogram_collection, degree_histogram_collection,
    ...
    c_index.lookup(
            label_histogram_collection, degree_histogram_collection, leaf_distance_histogram_collection, candidates,
            il_size_, threshold, ted_times
    );
    auto total_exec_time = duration_cast<std::chrono::milliseconds>(
            high_resolution_clock::now() - total_exec_time_start);
